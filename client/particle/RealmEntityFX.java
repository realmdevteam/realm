package mods.realm.client.particle;
...
public class RealmEntityFX extends EntityFX
{
	private static final ResourceLocation particles = new ResourceLocation("realm:/textures/misc/particles.png");
	
	String texturefile = null;
	int brightness = 200;
	
	public RealmEntityFX(World world, double x, double y, double z, double mx, double my, double mz)
	{
	...
	public void renderParticle(Tessellator tessellator, float tick, float rotationX, float rotationXZ, float rotationZ, float rotationYZ, float rotationXY)
	{
		if(this.texturefile != null)
			FMLClientHandler.instance().getClient().renderEngine.func_110577_a(particles);
		else
			FMLClientHandler.instance().getClient().renderEngine.func_110577_a(particles);
