...
public class RealmTeleporter extends Teleporter
{
	private final WorldServer worldServer;
    private final Random random;
    public static ArrayList<RealmPortalPosition> portals = new ArrayList<RealmPortalPosition>();
	
	public RealmTeleporter(WorldServer worldServer)
	{
		super(worldServer);
		...
		while(true){
			...
			for(int a=-2; a <= 2; a++)
			{
				for(int b=-2; b <= 2; b++)
				{
					...
					if(id == 0 || id == Block.waterStill.blockID || id == Block.waterMoving.blockID || id == Realm.waterStill.blockID || id == Realm.waterMoving.blockID || id == Block.blockNetherQuartz.blockID)
						waterBellow++;
				...
				for (int i = -2; i <= 2; i++) {
					for (int k = -2; k <= 2; k++) {
						...
						if(block == 0 || Block.blocksList[block].canBeReplacedByLeaves(worldServer, x+i, j, z+k)){
							...
							if(worldServer.provider.dimensionId == Realm.DimensionID)
								worldServer.setBlock(x+i, j, z+k, Realm.blockDirt.blockID);
							else
			...
			worldServer.setBlock(x, y+9, z, Realm.blockPortalCore.blockID);
