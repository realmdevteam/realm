...
public class RealmGenLakes extends WorldGenerator
{
	int liquidId;
	int lake[] = new int[2048];
	
	public RealmGenLakes(int liquidId)
	{
	...
	public boolean generate(World world, Random random, int x, int y, int z)
	{
		...
		for (cx = 0; cx < 16; cx++)
		{
			for (cz = 0; cz < 16; cz++)
			{
				for (cy = 0; cy < 8; cy++)
				{
					...
					{
						if(Block.blocksList[world.getBlockId(x+cx, y+cy, z+cz)] instanceof RealmBlock)
						{
							if(((RealmBlock)Block.blocksList[world.getBlockId(x+cx, y+cy, z+cz)]).canBeReplacedByLake())
		...
		if(isSurfaceLake && (Block.blocksList[this.liquidId].blockMaterial == Material.water))
		{
			...
			{
				new RealmGenSand(Realm.blockLeucosand.blockID, 7).generate(world, random, x+8, y+4, z+8);
			}
