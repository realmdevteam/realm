...
public class ChunkProviderRealm implements IChunkProvider
{
	/** RNG. */
	private Random rand;
	...
	public ChunkProviderRealm(World world, long par2)
	{
		this.worldObj = world;
					...
					for (int l1 = 0; l1 < 8; ++l1)
					{
						...
						for (int i2 = 0; i2 < 4; ++i2)
						{
							...
							for (int k2 = 0; k2 < 4; ++k2)
							{
								if ((d16 += d15) > 0.0D)
								{
									par3ArrayOfByte[j2 += short1] = (byte)Realm.blockGreatstone.blockID;
								}
								else if (k1 * 8 + l1 < seeLevel)
								{
									par3ArrayOfByte[j2 += short1] = (byte)Realm.waterStill.blockID;
								}
								else
								{
			...
			for (int x = 0; x < 16; ++x)
			{
				...
				for (int height = 127; height >= 0; --height)
				{
					...
					else
					{
						...
						else if (currentId == (byte)Realm.blockGreatstone.blockID)
						{
							if (j1 == -1)//if the top block is air
							{
								...
								{
									top = 0;
									filler = (byte)Realm.blockGreatstone.blockID;
								}
								...
								if (height < seeLevel && top == 0)
								{
									...
									{
										top = (byte)Realm.waterStill.blockID;
									}
								}
	...
	RealmGenLakes lakegenerator = new RealmGenLakes(Realm.waterStill.blockID);
	RealmGenLakes lavagenerator = new RealmGenLakes(Block.lavaStill.blockID);
	RealmGenSand sandgenerator = new RealmGenSand(Realm.blockLeucosand.blockID, 7);
	//RealmGenSand riltgenerator = new RealmGenSand(Realm.RiltBlock.blockID, 3);
	RealmGenRealmWood treegenerator = new RealmGenRealmWood(Realm.blockLeavesRealmWood.blockID, Realm.blockLogRealmWood.blockID, false);
	RealmGenFlowers flowergenerator = new RealmGenFlowers(Realm.blockBells.blockID, 0, 16, 8);
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
		...
		//TODO structures
		if(biomegenbase != Realm.biomeOcean)
		{
